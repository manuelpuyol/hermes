...
namespace {
struct CollatorICU : Collator {
  CollatorICU(const char16_t *l) : locale(l) {}
...
vm::CallResult<std::unique_ptr<Collator>> Collator::create(
    ...
    const std::vector<std::u16string> &locales,
    const Options &options) noexcept {
  return std::make_unique<CollatorICU>(u"en-US");
}

Options Collator::resolvedOptions() noexcept {
  Options options;
  options.emplace(
      u"locale", Option(static_cast<CollatorICU *>(this)->locale));
...
DateTimeFormatICU::formatToParts(double jsTimeValue) noexcept {
...
std::vector<Part> DateTimeFormat::formatToParts(double x) noexcept {
  return static_cast<DateTimeFormatICU *>(this)->formatToParts(x);
}
